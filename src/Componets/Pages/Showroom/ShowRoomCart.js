...
const ShowRoomCart = () => {
    const { subBrand } = useParams();
    const [brand, setBrand] = useState([]);


    useEffect(() => {
        fetch(`https://bikehourse.onrender.com/bike/showroom/${subBrand}`)
            .then(res => res.json())
        ...
        <section>
            {
                brand?.length === 0 ?
                    ...
                    <div className='container' style={{ marginTop: '120px' }}>
                        <h6 className='fw-bold fs-5 mb-3'><span className='text-capitalize'>{subBrand}</span> Showrooms available this Motorcycle</h6>
